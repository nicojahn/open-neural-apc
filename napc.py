...
class NeuralAPC():
    def __init__(self, *args, verbose=0, **kwargs):
        ...
        self._optimizer = keras.optimizers.Adam(self._training_parameter["learning_rate"],\
                                                     **self._training_parameter["optimizer_parameter"],\
                                                     **optimizer_clip_parameter)

        # helper for the loss
        self._TF_ZERO = K.cast(0., dtype=K.floatx())
        self._TF_ONE = K.cast(1., dtype=K.floatx())
        self._aux_scale = K.cast(self._training_parameter["aux_scale"], dtype=K.floatx())
        self._slack = K.cast(self._training_parameter["accuracy_error_niveau"], dtype=K.floatx())
    ...
    def _add_output(self):
        self._model.add(Dense(self._model_parameter["output_dimensions"], use_bias=True, name="OutputLayer"))
    ...
    def _add_callbacks(self):
        ...
        self._callbacks += [SaveEveryNthEpochCustom(self, self._training_parameter["safe_steps"])]
    ...
    def accuracy(self, y_true, y_pred):
        ...
        error_with_slack = K.cast(K.less_equal(error_with_slack, self._TF_ZERO), dtype=K.floatx())
    ...
    def _aux_losses(mask, prediction, TF_ZERO, TF_ONE):
        ...
        stabelize_change_forward = K.maximum(TF_ZERO, \
